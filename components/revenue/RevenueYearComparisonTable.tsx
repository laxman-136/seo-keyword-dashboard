// components/revenue/RevenueYearComparisonTable.tsx
import React from 'react'
import { RevenueYearlyDetailRow } from '@/lib/types'
import { getRevenueYearComparison, formatCurrency } from '@/lib/sheets'
import { ArrowUpRight, ArrowDownRight, Minus } from 'lucide-react'
import { cn } from '@/lib/utils'

interface RevenueYearComparisonTableProps {
  rows: RevenueYearlyDetailRow[]
  yearA: string
  yearB: string
}

export default function RevenueYearComparisonTable({ rows, yearA, yearB }: RevenueYearComparisonTableProps) {
  const { a, b, deltas } = getRevenueYearComparison(rows, yearA, yearB)
  ...
  return (
    <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden w-full">
      <div className="px-6 py-4 border-b border-slate-100 bg-slate-50/50">
        <h3 className="font-bold text-slate-800 text-sm">📅 Yearly Period Comparison</h3>
        <p className="text-xs text-slate-400 mt-0.5">Side-by-side yearly performance audit between selected years</p>
      </div>
        ...
        <table className="w-full text-sm text-left">
          <thead>
            <tr className="bg-slate-900 text-slate-200 text-xs font-bold uppercase tracking-wider border-b border-slate-700">
              ...
              <th className="px-4 py-3.5 text-right">{yearA}</th>
              <th className="px-4 py-3.5 text-right">{yearB}</th>
