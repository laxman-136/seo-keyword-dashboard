// components/revenue/RevenueCourseDonut.tsx
'use client'
...
import { REVENUE_COURSE_COLORS, formatCurrency } from '@/lib/sheets'

interface RevenueCourseDonutProps {
  courses: RevenueCourseAggregate[]
}

export default function RevenueCourseDonut({ courses }: RevenueCourseDonutProps) {
  const totalRevenue = courses.reduce((acc, c) => acc + c.revenue, 0)

  const data = courses.map(c => ({
    name: c.courseName,
    value: c.revenue,
    color: REVENUE_COURSE_COLORS[c.courseName] || '#6b7280',
    percentLabel: `${c.revenueSharePct.toFixed(1)}%`
  })).filter(d => d.value > 0)
    ...
    <div className="bg-white p-5 md:p-6 rounded-2xl border border-slate-200 shadow-sm min-h-[380px] flex flex-col justify-between relative">
      <div>
        <h4 className="text-sm font-bold text-slate-800 tracking-tight">
          Revenue Share by Course
        </h4>
        <p className="text-xs text-slate-400 mt-1">
          Revenue contribution breakdown across IT hub course offerings
        </p>
      </div>
