// components/revenue/RevenueYearOrganicPaidCard.tsx
import React from 'react'
import { RevenueYearlyDetailRow } from '@/lib/types'
import { formatCurrency } from '@/lib/sheets'

interface RevenueYearOrganicPaidCardProps {
  row: RevenueYearlyDetailRow
}

export default function RevenueYearOrganicPaidCard({ row }: RevenueYearOrganicPaidCardProps) {
  ...
  return (
    <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6 space-y-6">
      <div>
        <h3 className="font-bold text-slate-800 text-sm">⚖️ Performance Source Comparison (Yearly)</h3>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="bg-slate-50/50 p-4 rounded-xl border border-slate-100 flex flex-col justify-between">
          <div>
            ...
            <p className="text-[10px] text-slate-500 mt-0.5">Share: {((organicRev / totalRev) * 100).toFixed(1)}%</p>
          </div>
        ...
        <div className="bg-slate-50/50 p-4 rounded-xl border border-slate-100 flex flex-col justify-between">
          <div>
            ...
            <p className="text-[10px] text-slate-500 mt-0.5">Share: {((paidRev / totalRev) * 100).toFixed(1)}%</p>
          </div>
          <div className="mt-4 pt-3 border-t border-slate-200/60 text-xs font-semibold text-slate-600 flex justify-between">
        ...
        <div className="bg-slate-50/50 p-4 rounded-xl border border-slate-100 flex flex-col justify-between">
          <div>
            ...
            <p className="text-[10px] text-slate-500 mt-0.5">Share: {((webRev / totalRev) * 100).toFixed(1)}%</p>
          </div>
          <div className="mt-4 pt-3 border-t border-slate-200/60 text-xs font-semibold text-slate-600 flex justify-between">
