// components/revenue/RevenueKPICard.tsx
import React from 'react'
import { TrendingUp, TrendingDown, Minus } from 'lucide-react'
import { cn } from '@/lib/utils'
import { formatCurrency } from '@/lib/sheets'

export type RevenueKPICardVariant = 'blue' | 'indigo' | 'green' | 'emerald' | 'amber' | 'orange' | 'purple' | 'red' | 'gray'
...
interface RevenueKPICardProps {
  ...
  prevValue?: number
  icon: string
  variant: RevenueKPICardVariant
  isPercent?: boolean
  isCurrency?: boolean
  subtitle?: string
}

const STYLES: Record<RevenueKPICardVariant, {
  gradient: string
  iconBg: string
  accentBar: string
  valueText: string
  ...
  orange:  { gradient: 'from-orange-50 to-white',  iconBg: 'bg-orange-100',  accentBar: 'bg-orange-400',  valueText: 'text-orange-900', badgePos: 'bg-emerald-50 border-emerald-200 text-emerald-700', badgeNeg: 'bg-red-50 border-red-200 text-red-700' },
...
export default function RevenueKPICard({
  ...
  const formatDelta = (d: number) => {
    ...
    if (isPercent) return `${d > 0 ? '+' : '-'}${abs.toFixed(1)}pp`
    if (isCurrency) return `${d > 0 ? '+' : '-'}${formatCurrency(abs)}`
    return `${d > 0 ? '+' : '-'}${abs.toLocaleString()}`
  }
