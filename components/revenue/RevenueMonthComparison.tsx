// components/revenue/RevenueMonthComparison.tsx
import React from 'react'
import { RevenueMonthlyRow } from '@/lib/types'
import { getRevenueMonthComparison, formatCurrency } from '@/lib/sheets'
import { ArrowUpRight, ArrowDownRight, Minus } from 'lucide-react'
import { cn } from '@/lib/utils'

interface RevenueMonthComparisonProps {
  rows: RevenueMonthlyRow[]
  monthA: string
  monthB: string
}

export default function RevenueMonthComparison({ rows, monthA, monthB }: RevenueMonthComparisonProps) {
  const { a, b, deltas } = getRevenueMonthComparison(rows, monthA, monthB)
  ...
  const metrics = [
    { label: '💰 Gross Revenue', valA: a.totalRevenue, valB: b.totalRevenue, delta: deltas.totalRevenue, key: 'totalRevenue', isCurrency: true, highlight: 'text-emerald-600 font-extrabold' },
    { label: '🎓 Student Enrollments', valA: a.totalConversions, valB: b.totalConversions, delta: deltas.totalConversions, key: 'totalConversions', highlight: 'text-blue-600 font-bold' },
    { label: '🎫 Average Ticket Size', valA: a.avgRevenuePerStudent, valB: b.avgRevenuePerStudent, delta: deltas.avgRevenuePerStudent, key: 'avgRevenuePerStudent', isCurrency: true },
    ...
    { label: '📢 Total Ad Spend', valA: a.totalAdSpend, valB: b.totalAdSpend, delta: deltas.totalAdSpend, key: 'totalAdSpend', isCurrency: true, highlight: 'text-red-650' },
  ...
  return (
    <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden w-full">
      <div className="px-6 py-4 border-b border-slate-100 bg-slate-50/50">
        <h3 className="font-bold text-slate-800 text-sm">📅 Revenue Period Comparison</h3>
        <p className="text-xs text-slate-400 mt-0.5">Side-by-side financial and campaign performance audit between selected months</p>
      </div>
        ...
        <table className="w-full text-sm text-left">
          ...
          <tbody className="divide-y divide-slate-100">
            {metrics.map(m => {
              ...
              // Color indicators (Reverse colors for Total Ad Spend)
              ...
              return (
                <tr key={m.key} className="hover:bg-slate-50/60 transition-colors">
                  ...
                  <td className={cn("px-6 py-4 text-right font-mono font-bold", 
                    ...
                    isNegative ? (isSpendMetric ? 'text-emerald-600' : 'text-red-500') : 'text-slate-505'
                  )}>
                    {pctChange}
                  </td>
                </tr>
