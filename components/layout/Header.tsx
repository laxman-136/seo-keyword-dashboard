...
export default function Header({
  ...
  return (
    <header className="bg-white border-b border-slate-200 px-8 py-5 flex items-center justify-between shadow-sm relative z-10 no-print">
      {/* Title & Timing info */}
      <div>
      ...
      <div className="flex items-center gap-3">
        {onRefresh && (
          <button
            ...
            className="flex items-center gap-2 px-4 py-2 border border-slate-200 hover:border-slate-300 bg-white hover:bg-slate-50 text-slate-600 hover:text-slate-800 font-medium text-sm rounded-xl transition-all shadow-sm disabled:opacity-50"
          >
        ...
        <button
          onClick={handlePrint}
          className="flex items-center gap-2 px-4 py-2 bg-slate-900 hover:bg-slate-800 text-white font-medium text-sm rounded-xl transition-all shadow-md shadow-slate-900/10 hover:shadow-slate-950/20"
        >
