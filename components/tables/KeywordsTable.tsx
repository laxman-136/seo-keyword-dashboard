...
export default function KeywordsTable({ keywords, groups }: KeywordsTableProps) {
  ...
  return (
    <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden flex flex-col justify-between">
      {/* Filtering panel */}
      <div className="p-6 border-b border-slate-100 space-y-4 no-print">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          <span className="text-xs text-slate-400 font-medium">
            Showing {processedData.length} of {keywords.length} keywords
          </span>
        </div>
        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="bg-slate-50/75 border-b border-slate-100 text-xs font-bold text-slate-500 uppercase tracking-wider select-none">
              <th 
                className="px-6 py-4 cursor-pointer hover:bg-slate-100/50 hover:text-slate-700 transition-all"
