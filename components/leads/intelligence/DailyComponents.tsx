...
export function DailyKPICards({ todayLeads, yesterdayLeads, todayContacted, pendingCallback, urgentUncontacted, todayEnrolled }: DailyKPICardsProps) {
  ...
  return (
    <div className="grid grid-cols-2 lg:grid-cols-6 gap-3">
      ...
      <div className="col-span-1 bg-white p-5 rounded-2xl border border-blue-100 bg-blue-50/20 shadow-sm">
        <div className="flex items-center justify-between mb-3">
        ...
        <p className="text-[11px] text-blue-400 mt-1">{todayLeads > 0 ? ((todayContacted/todayLeads)*100).toFixed(0) : 0}% contact rate</p>
      </div>
...
export function TeamPerformanceTable({ agents }: TeamPerformanceTableProps) {
  const sorted = [...agents].sort((a, b) => b.enrolled - a.enrolled)
  return (
    <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
            ...
            {sorted.map((a, idx) => (
              <tr key={a.agent} className="hover:bg-slate-50/50">
                ...
                <td className="py-3 px-5 flex items-center gap-2">
                  ...
                  <span className="font-bold text-slate-800 truncate max-w-[120px]">{a.agent.split('@')[0]}</span>
                </td>
