...
export default function LeadsMonthSelector({
  months,
  selected,
  onChange,
  selectedCompare,
  onChangeCompare,
  label = 'Analyze Month'
}: LeadsMonthSelectorProps) {
  const [compareOpen, setCompareOpen] = useState(false)
  const [mainOpen, setMainOpen] = useState(false)
  const compareRef = useRef<HTMLDivElement>(null)
  const mainRef = useRef<HTMLDivElement>(null)

  // Use pill tabs when ≤6 months, dropdown when more
            ...
            {months.map((m, idx) => {
              ...
              return (
                <button
                  ...
                  title={m}
                  className={cn(
                    'relative flex flex-col items-center justify-center px-3.5 py-2 rounded-xl text-center transition-all duration-200 select-none focus:outline-none',
                    isActive
                      ? 'bg-gradient-to-br from-violet-600 to-indigo-600 text-white shadow-lg shadow-violet-500/25 scale-[1.03]'
                      : 'text-slate-500 hover:text-slate-800 hover:bg-white hover:shadow-sm'
                  )}
                >
                  {isLatest && !isActive && (
                    <span className="absolute -top-1.5 -right-1 text-[7px] font-extrabold bg-emerald-500 text-white px-1 py-[1px] rounded-full leading-none tracking-wide shadow-sm">
                      NEW
                    </span>
                  )}
                  <span className={cn(
                  ...
                  <span className={cn(
                    'text-[10px] font-medium mt-[3px] leading-none',
        ...
        {!usePills && (
          <div ref={mainRef} className="relative">
            <button
              ...
              className={cn(
                'flex items-center gap-2.5 pl-4 pr-3 py-2.5 rounded-xl border text-sm font-bold transition-all duration-150 min-w-[160px] shadow-sm',
                mainOpen
      ...
      {onChangeCompare && selectedCompare !== undefined && (
        <div ref={compareRef} className="relative flex flex-col gap-1.5">
          ...
          <button
            ...
            className={cn(
              'flex items-center gap-2.5 pl-4 pr-3 py-2.5 rounded-xl border text-sm font-bold transition-all duration-150 min-w-[150px] shadow-sm',
              compareOpen
