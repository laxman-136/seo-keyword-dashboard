...
async function run() {
  try {
    ...
    // Scan lead_date start dates from March 1 to March 31
    for (let day = 1; day <= 31; day++) {
      const fromTime = new Date(`2026-03-${String(day).padStart(2, '0')}T00:00:00+05:30`).getTime();
      const count = await testSearch(token, enterpriseId, { lead_date: { from: fromTime, to: toTime } });
      console.log(`Lead Date from March ${day}: ${count}`);
    }
