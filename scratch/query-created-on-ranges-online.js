...
async function run() {
  try {
    ...
    // Scan different start dates for created_on to see if any matches 1735
    for (let day = 1; day <= 31; day++) {
      const fromTime = new Date(`2026-03-${String(day).padStart(2, '0')}T00:00:00+05:30`).getTime();
      const count = await testSearch(token, enterpriseId, { created_on: { from: fromTime, to: toTime } });
      console.log(`Created On from March ${day}: ${count}`);
    }
