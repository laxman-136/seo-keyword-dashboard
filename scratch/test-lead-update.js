...
async function run() {
  try {
    ...
    const leadId = '6a23c7d89f80fd2c2234fc0a'; // Abhishek Singh
    ...
    const response = await fetch(url, {
      ...
      body: JSON.stringify({ fields: { phone: '919354015698' } })
    });
    const res = await response.json();
    const lead = res.data.find(l => l.id === leadId);
    console.log('Updated Lead details:', JSON.stringify(lead.fields, null, 2));
