...
async function run() {
  try {
    ...
    console.log(`Searching with $in operator...`);
    const res = await testSearch({ email: { $in: ['daniyalmujawar4321@gmail.com'] } });
    console.log('Results length:', res.data.length);
    if (res.data.length > 0) {
      console.log('Results data:', JSON.stringify(res.data.map(l => ({ name: l.fields.name, email: l.fields.email })), null, 2));
    }
