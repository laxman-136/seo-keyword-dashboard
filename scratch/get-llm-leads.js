...
async function run() {
  try {
    ...
    // Fetch leads created in June (by lead_date)
    ...
    // Fetch leads created in June (by created_on)
    let juneCreatedOn = [];
    skip = 0;
    while (true) {
    ...
    const llmLeads = allLeads.filter(lead => detectLeadChannel(lead) === 'LLM');

    console.log(`\n--- June LLM Leads (${llmLeads.length}) ---`);
    llmLeads.forEach((l, idx) => {
      ...
      const utmSource = l.fields?.utmsource || 'N/A';
      const leadSource = l.fields?.lead_source_1 || 'N/A';

      console.log(`[${idx + 1}] Name: ${l.fields?.name} | Email: ${email} | Phone: ${phone} | Course: ${course} | Status: ${l.status} | Lead Date: ${leadDate} | Enroll Date: ${enrollDate} | UTM Source: ${utmSource} | Source: ${leadSource}`);
    });
