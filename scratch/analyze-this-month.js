...
async function run() {
  try {
    ...
    const channelStats = {};
    const sourceStats = {};
    const websiteLeadsBreakdown = [];

    filteredLeads.forEach(l => {
      const ch = detectChannel(l);
      const src = l.fields?.lead_source_1 || '(no source)';
      
      channelStats[ch] = (channelStats[ch] || 0) + 1;
      sourceStats[src] = (sourceStats[src] || 0) + 1;

      if (ch === 'Website' || src.toLowerCase() === 'website') {
        websiteLeadsBreakdown.push({
          name: l.fields?.name,
          source: src,
          channel: ch,
          utm_source: l.fields?.utmsource,
          utm_medium: l.fields?.utmmedium,
          utm_campaign: l.fields?.utmcampaign,
          gclid: l.fields?.google_gcl_id || l.fields?.gclid,
          fbclid: l.fields?.fbclid,
          date: new Date(l.fields?.lead_date || l.fields?.created_on).toLocaleDateString()
        });
      }
    });
    ...
    console.log('\n--- WEBSITE CHANNEL LEADS DETAILS ---');
    console.log(JSON.stringify(websiteLeadsBreakdown.filter(w => w.channel === 'Website'), null, 2));
