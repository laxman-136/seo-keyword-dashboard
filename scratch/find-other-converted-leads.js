...
async function run() {
  try {
    ...
    console.log('\n--- June Enrolled Leads (Non-Organic / Non-Google Ads) ---');

    juneEnrolledLeads.forEach((l) => {
      const channel = detectLeadChannel(l);
      if (channel === 'Organic' || channel === 'Google Ads') return;

      const name = l.fields?.name;
      const email = l.fields?.email;
      const phone = l.fields?.phone;
      ...
      console.log(`- Channel: [${channel}] | Name: ${name} | Email: ${email} | Phone: ${phone} | Enroll Date: ${enrollDate} | Source Field: ${leadSource}`);
    });
