  ...
  const fetchData = useCallback(async (bypassCache = false) => {
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(errData.error || 'Failed to fetch course ads data')
      }
    ...
    <div className="min-h-screen bg-slate-50/50 p-4 sm:p-6 lg:p-8 space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-lg font-bold text-slate-800 leading-none">Course Blended Ad Analytics</h1>
            <p className="text-xs text-slate-400 mt-1">Blended client acquisition cost (CPE) and ROI mapped by Oracle training courses</p>
          </div>
        </div>
      ...
      {error && (
        <div className="flex items-center gap-2.5 p-4 bg-rose-50 border border-rose-200 rounded-2xl text-rose-700 text-xs font-semibold">
          ...
          <span>Error loading Course Ads: {error}</span>
        </div>
      )}
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
          <div className="p-5 border-b border-slate-100">
            <h4 className="text-base font-bold text-slate-800">Course ERP Marketing Index</h4>
            <p className="text-xs text-slate-400">Blended spends, leads, close rates and overall true ROI per module</p>
          </div>
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-slate-50 text-slate-400 text-[10px] uppercase font-bold tracking-wider border-b border-slate-100">
                  <th className="py-2.5 px-4">ERP Training Module</th>
                  ...
                  <th className="py-2.5 px-4 text-center text-emerald-600">True ROAS</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-100 text-xs font-semibold text-slate-700">
                {data.courses.map((item: any) => (
                  <tr key={item.course} className="hover:bg-slate-50/50">
                    <td className="py-3 px-4 text-slate-850 font-bold">{item.course}</td>
                    ...
                    <td className="py-3 px-4 text-center font-bold text-slate-800">{item.enrolled}</td>
                    <td className="py-3 px-4 text-center">
                      <span className="inline-flex px-2 py-0.5 rounded-full bg-emerald-50 text-emerald-700 font-bold">{item.trueROAS}x</span>
                    </td>
                  </tr>
                ))}
              </tbody>
