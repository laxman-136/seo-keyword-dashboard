...
import { FullFunnelVisualizer } from '@/components/ads/intelligence/AdsIntelComponents'
    ...
    <div className="min-h-screen bg-slate-50/50 p-4 sm:p-6 lg:p-8 space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-lg font-bold text-slate-800 leading-none">Paid Funnel Leak Detection</h1>
            <p className="text-xs text-slate-400 mt-1">Overlay impressions and clicks with CRM demo attendance and student enrollments</p>
          </div>
        </div>
      ...
      {error && (
        <div className="flex items-center gap-2.5 p-4 bg-rose-50 border border-rose-200 rounded-2xl text-rose-700 text-xs font-semibold">
          ...
          <span>Error loading Funnel Leak: {error}</span>
        </div>
      )}
      ...
      {loading ? (
        ...
        <div className="space-y-6">
          <FullFunnelVisualizer
            impressions={data.impressions}
            clicks={data.clicks}
            leadsCRM={data.leadsCRM}
            demos={data.demos}
