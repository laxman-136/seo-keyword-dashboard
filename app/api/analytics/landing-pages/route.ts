...
import { fetchGA4LandingPages, fetchGA4SourceLandingMatrix } from '@/lib/ga4-api'
...
export async function GET(request: Request) {
  try {
    ...
    const [landingPages, sourceLandingMatrix] = await Promise.all([
      fetchGA4LandingPages(dateRange, bypassCache),
      fetchGA4SourceLandingMatrix(dateRange, bypassCache)
    ])
    ...
    console.error('GA4 Landing Pages API error:', error)
