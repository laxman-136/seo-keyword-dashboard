...
import { fetchGA4PagePaths, fetchGA4ReturningUsers } from '@/lib/ga4-api'
...
export async function GET(request: Request) {
  try {
    ...
    const [pagePaths, returningUsers] = await Promise.all([
      fetchGA4PagePaths(dateRange, bypassCache),
      fetchGA4ReturningUsers(dateRange, bypassCache)
    ])
    ...
    console.error('GA4 User Journey API error:', error)
