...
export async function GET(request: Request) {
  try {
    ...
    if (!adSetId) {
      return NextResponse.json({ error: 'Missing adSetId parameter' }, { status: 400 })
    }
    ...
    const cacheKey = `meta_ads_${adSetId}_${dateRange.from}_${dateRange.to}_${dateRange.preset}_${metaAdAccountId || 'default'}`

    const cacheResult = await getOrSetCache(
      cacheKey,
      () => fetchMetaAds(adSetId, dateRange, metaAdAccountId, metaAccessToken),
      bypassCache
    )
    ...
    console.error('Meta Ads list API Route error:', error)
    return NextResponse.json(
