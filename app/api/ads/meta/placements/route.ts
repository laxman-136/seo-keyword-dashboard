// app/api/ads/meta/placements/route.ts
...
import { fetchMetaPlacements } from '@/lib/meta-api'
...
export async function GET(request: Request) {
  try {
    ...
    const cacheKey = `meta_placements_${dateRange.from}_${dateRange.to}_${dateRange.preset}_${metaAdAccountId || 'default'}`

    const cacheResult = await getOrSetCache(
      cacheKey,
      () => fetchMetaPlacements(dateRange, metaAdAccountId, metaAccessToken),
      bypassCache
    )

    return NextResponse.json({
      placements: cacheResult.data,
    ...
    console.error('Meta Placements API Route error:', error)
    return NextResponse.json(
