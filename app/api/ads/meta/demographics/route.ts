// app/api/ads/meta/demographics/route.ts
...
import { fetchMetaDemographics } from '@/lib/meta-api'
...
export async function GET(request: Request) {
  try {
    ...
    const cacheKey = `meta_demographics_${dateRange.from}_${dateRange.to}_${dateRange.preset}_${metaAdAccountId || 'default'}`

    const cacheResult = await getOrSetCache(
      cacheKey,
      () => fetchMetaDemographics(dateRange, metaAdAccountId, metaAccessToken),
      bypassCache
    )

    return NextResponse.json({
      demographics: cacheResult.data,
    ...
    console.error('Meta Demographics API Route error:', error)
    return NextResponse.json(
