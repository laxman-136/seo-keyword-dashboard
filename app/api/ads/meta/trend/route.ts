// app/api/ads/meta/trend/route.ts
...
import { fetchMetaDailyTrend } from '@/lib/meta-api'
...
export async function GET(request: Request) {
  try {
    ...
    const cacheKey = `meta_trend_${dateRange.from}_${dateRange.to}_${dateRange.preset}_${metaAdAccountId || 'default'}`

    const cacheResult = await getOrSetCache(
      cacheKey,
      () => fetchMetaDailyTrend(dateRange, metaAdAccountId, metaAccessToken),
      bypassCache
    )

    return NextResponse.json({
      trend: cacheResult.data,
    ...
    console.error('Meta Daily Trend API Route error:', error)
    return NextResponse.json(
