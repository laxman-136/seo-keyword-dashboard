// app/api/ads/meta/adsets/route.ts
...
export async function GET(request: Request) {
  try {
    ...
    const cacheKey = `meta_adsets_${campaignId}_${dateRange.from}_${dateRange.to}_${dateRange.preset}_${metaAdAccountId || 'default'}`

    const cacheResult = await getOrSetCache(
      cacheKey,
      () => fetchMetaAdSets(campaignId, dateRange, metaAdAccountId, metaAccessToken),
      bypassCache
    )

    return NextResponse.json({
      adSets: cacheResult.data,
    ...
    console.error('Meta Adsets API Route error:', error)
    return NextResponse.json(
