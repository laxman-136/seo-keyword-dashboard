// app/api/ads/google/ads/route.ts
...
export async function GET(request: Request) {
  try {
    ...
    if (!adGroupId) {
      return NextResponse.json({ error: 'Missing adGroupId parameter' }, { status: 400 })
    }
    ...
    const cacheKey = `google_ads_${adGroupId}_${dateRange.from}_${dateRange.to}_${dateRange.preset}_${googleCustomerId || 'default'}`

    const cacheResult = await getOrSetCache(
      cacheKey,
      () => fetchGoogleAds(adGroupId, dateRange, googleDevToken, googleClientId, googleClientSecret, googleRefreshToken, googleCustomerId, googleManagerId),
      bypassCache
    )

    return NextResponse.json({
      ads: cacheResult.data,
    ...
    console.error('Google Ads list API Route error:', error)
    return NextResponse.json(
