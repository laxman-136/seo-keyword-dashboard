// app/api/ads/google/geo/route.ts
...
import { fetchGoogleGeoBreakdown } from '@/lib/google-ads-api'
...
export async function GET(request: Request) {
  try {
    ...
    const cacheKey = `google_geo_${dateRange.from}_${dateRange.to}_${dateRange.preset}_${googleCustomerId || 'default'}`

    const cacheResult = await getOrSetCache(
      cacheKey,
      () => fetchGoogleGeoBreakdown(dateRange, googleDevToken, googleClientId, googleClientSecret, googleRefreshToken, googleCustomerId, googleManagerId),
      bypassCache
    )

    return NextResponse.json({
      locations: cacheResult.data.locations,
    ...
    console.error('Google Geo Breakdown API Route error:', error)
    return NextResponse.json(
