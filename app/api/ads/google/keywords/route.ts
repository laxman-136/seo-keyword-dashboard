// app/api/ads/google/keywords/route.ts
...
import { fetchGoogleKeywords } from '@/lib/google-ads-api'
...
export async function GET(request: Request) {
  try {
    ...
    const cacheKey = `google_keywords_${dateRange.from}_${dateRange.to}_${dateRange.preset}_${googleCustomerId || 'default'}`

    const cacheResult = await getOrSetCache(
      cacheKey,
      () => fetchGoogleKeywords(dateRange, googleDevToken, googleClientId, googleClientSecret, googleRefreshToken, googleCustomerId, googleManagerId),
      bypassCache
    )

    return NextResponse.json({
      keywords: cacheResult.data,
    ...
    console.error('Google Keywords API Route error:', error)
    return NextResponse.json(
