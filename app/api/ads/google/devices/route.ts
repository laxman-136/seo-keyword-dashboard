// app/api/ads/google/devices/route.ts
...
import { fetchGoogleDeviceBreakdown } from '@/lib/google-ads-api'
...
export async function GET(request: Request) {
  try {
    ...
    const cacheKey = `google_devices_${dateRange.from}_${dateRange.to}_${dateRange.preset}_${googleCustomerId || 'default'}`
    ...
    return NextResponse.json({
      devices: cacheResult.data.devices,
    ...
    console.error('Google Devices Breakdown API Route error:', error)
    return NextResponse.json(
