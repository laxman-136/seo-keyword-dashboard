// app/api/ads/google/adgroups/route.ts
...
import { fetchGoogleAdGroups } from '@/lib/google-ads-api'
...
export async function GET(request: Request) {
  try {
    ...
    const cacheKey = `google_adgroups_${campaignId}_${dateRange.from}_${dateRange.to}_${dateRange.preset}_${googleCustomerId || 'default'}`

    const cacheResult = await getOrSetCache(
      cacheKey,
      () => fetchGoogleAdGroups(campaignId, dateRange, googleDevToken, googleClientId, googleClientSecret, googleRefreshToken, googleCustomerId, googleManagerId),
      bypassCache
    )

    return NextResponse.json({
      adGroups: cacheResult.data,
    ...
    console.error('Google Ad Groups API Route error:', error)
    return NextResponse.json(
