...
import RevenueSourceTable from '@/components/revenue/RevenueSourceTable'
import RevenueSourceDonut from '@/components/revenue/RevenueSourceDonut'
import RevenueSourceTrend from '@/components/revenue/RevenueSourceTrend'
import { getAvailableRevenueMonths, getRevenueSourceBreakdown } from '@/lib/sheets'
...
export default function RevenueSourcesPage() {
    ...
    return (
      <div className="p-6 flex-1 flex flex-col items-center justify-center bg-slate-50 min-h-screen">
        <div className="max-w-md w-full bg-white p-8 rounded-2xl border border-red-100 shadow-xl flex flex-col items-center text-center">
          ...
          <h2 className="text-xl font-bold text-slate-800">No source revenue data</h2>
          <p className="text-slate-400 text-sm mt-3">
            {error || 'Please fill in the revenue source fields in Google Sheets to load the dashboard.'}
          </p>
          <button
            onClick={() => refresh()}
  ...
  const sourceBreakdown = getRevenueSourceBreakdown(monthly, currentMonth)
  ...
  return (
    <div className="w-full max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-6 lg:space-y-8 min-h-screen bg-slate-50">
      ...
      <div className="bg-white border border-slate-200 rounded-2xl px-5 py-4 shadow-sm flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <p className="text-sm font-bold text-slate-700">Source Analysis Month</p>
          <p className="text-xs text-slate-400 mt-0.5">Select a month to see acquisition channels and conversions share splits</p>
        </div>
      ...
      {/* ── SECTION A: Table & Donut ── */}
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-5 items-stretch">
        <div className="lg:col-span-7">
          <RevenueSourceTable sources={sourceBreakdown} />
        </div>
        <div className="lg:col-span-5">
          <RevenueSourceDonut sources={sourceBreakdown} />
        </div>
      </div>

      {/* ── SECTION B: Area Chart ── */}
      <RevenueSourceTrend rows={monthly} />
    </div>
  )
}
